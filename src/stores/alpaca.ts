...
export type AlpacaAccessorize = 'backgrounds' | 'neck' | 'leg' | 'ears' | 'hair' | 'eyes' | 'accessories' | 'mouth'
...
export const alpacaStore = reactive<AlpacaStore>({
  ...
  shuffle() {
    for (const accessorize in this.accessorizes) {
      this.accessorizes[accessorize as AlpacaAccessorize] = shuffleAlpacaAccessorize(accessorize as AlpacaAccessorize)
    }
  }
})